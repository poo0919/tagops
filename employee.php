...
<div id="page-wrapper" class="container">
  <div class="bs-example">
    <center>   
    <ul class="nav nav-pills" style="display:inline-block;" id="adminEmployeeManagementTabs" >
      <li class="active " ><a data-toggle="tab" href="#particularEmployeeTab" >
      <?php
        $user_id=$_GET['id'];
    ...
    <div class="tab-content" id="myContent">
      <div id="particularEmployeeTab" class="tab-pane fade in active">
        <form class="form-inline"  id="form-sort" style="float:left;" >
          <div class="form-group" >
            <label style="color: #2a409f;">Sort </label>
            <select  class="form-control" style="background: #fcf9f9;" id="sort">
        ...
        <form class="form-inline"  id="form-sort" style="float:left;margin-left: 10px;" >
          <div class="form-group" >
            <label style="color: #2a409f;">Status </label>
            <select  class="form-control" style="background: #fcf9f9;" id="filter2-employees">
        ...
        <table class="table table-bordered table-hover table-condensed" >
          <thead>
            <tr> <th>S.No.</th> <th>Project</th> <th>Amount</th> <th>Date</th> <th>Details</th> <th>Category</th> <th>Payment Mode</th> <th>Bill</th> <th>View </th> <th>Status</th> <th>Action</th>
            </tr>
          </thead>
          <tbody>
            <?php
              if(!isset($_GET['sort']) && !isset($_GET['filter2-employees'])){
                $user_id=$_GET['id'];
                $query = "Select id,project_id,amount,date,details,category_id,payment_id,bill,reimbursed,file from data where user_id='$user_id' AND reimbursed='0' order by DATE(date) DESC";
                $result=mysqli_query($conn,$query)or die(mysqli_error($conn));
                if ($result->num_rows > 0) {
                  $index=1;
                  while ($row = $result->fetch_array()){                                
                    ...
                    if($reimbursed=="Submitted"){
                      echo "</td><td style='color:#71D3f4;'>".$reimbursed."</td><td>";
                    }else if($reimbursed=="Approved"){
                      echo "</td><td style='color:#7cc576;'>".$reimbursed."</td><td>";
                    }else if($reimbursed=="Rejected"){
                      echo "</td><td style='color:#fea862;'>".$reimbursed."</td><td>";
                    }
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#filter2-employees').change(function(){
      ...
      window.location.href="employee.php?filter2-employees="+$('#filter2-employees').val()+"&id="+idVariable;
    });
    ...
    $('#sort').change(function(){
      ...
      window.location.href="employee.php?sort="+$('#sort').val()+"&id="+idVariable;
    });
